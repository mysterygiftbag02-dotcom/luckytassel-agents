...
const prompts = {
  ...
  blog: "Write a 4-sentence intro paragraph for a Lucky Tassel blog post titled '5 Ways to Style Tassels This Season'. Fun, trend-aware tone. Reply with ONLY the paragraph.",
  ...
  try {
    ...
    if (agentType === 'product') {
      ...
      const r2 = await fetch(`${base}/blogs/${bid}/articles.json`, {
        method: 'POST', headers,
        body: JSON.stringify({ article: { title: '5 Ways to Style Tassels This Season', body_html: `<p>${content}</p>`, published: false } })
      });
      ok = r2.ok;
      ...
      if (page) {
        ...
        const r2 = await fetch(`${base}/pages.json`, {
          method: 'POST', headers,
          body: JSON.stringify({ page: { title: 'Returns & FAQ', body_html: `<p>${content}</p>`, p
